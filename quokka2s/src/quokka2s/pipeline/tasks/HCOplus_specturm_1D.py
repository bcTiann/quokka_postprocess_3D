...
class HCOplusLine1DTask(AnalysisTask):
    ...
    def compute(self, context: PipelinePlotContext):
        ...
        nu_0 = self._HCOplus_freq_field[0, 0, 0].in_units("Hz")

        freq_3d = self._HCOplus_freq_field.in_units("Hz")
        
        shifted_freq_3d = (self._HCOplus_freq_field * self._Bulk_Doppler_factor_x).in_units("Hz")
        ...
        out_path = self.config.output_dir / "HCO+_delta_freq_yz_surface.png"
        plt.savefig(str(out_path), dpi=600)
        print(f"Saved: {out_path}")

        lum_3d = (self._HCOplus_lum_3d * self._volume_3d).in_units("erg/s")

        thermal_3d = self._HCOplus_thermal_width.in_units("cm/s")
        ...
        print(f"  [HCO+] building spectral cube (erf integration, {nx} slices) ...")
        ...
        out_path = self.config.output_dir / "HCO+_Surface_Brightness.png"
        ...
        fig.suptitle(f"HCO+ Spectral Grid Map", fontsize=18, y=0.92)
        ...
        out_path = self.config.output_dir / "HCO+_Spectral_Grid.png"
