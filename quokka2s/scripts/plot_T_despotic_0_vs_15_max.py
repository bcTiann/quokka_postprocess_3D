#!/usr/bin/env python
"""T_DESPOTIC L_ext = 0 vs 15 kpc, with MAX-of-6-directions column density.
Per slice: [T_DSP@0, T_DSP@15, plain ratio T_DSP@0 / T_DSP@15], 4 slices.
Reads the (max) TemperatureSlicesTask caches. Full-range T colourbars.
"""
import glob, os
...
fig.suptitle('T_DESPOTIC  L_ext 0 vs 15 kpc  (MAX-of-6 column density, full-range T)\n'
             'ratio>1 = cooler at L_ext=15', fontsize=12, y=0.93)

OUT = ROOT / 'plt0655228_down1_LextDiff_0kpc_vs_9kpc'
OUT.mkdir(parents=True, exist_ok=True)
out = OUT / 'T_DESPOTIC_max_0_vs_15.png'
...
for i in range(n):
    ...
    print(f'  idx {int(sidx[i])}: T0/T15 median={np.median(r):.3f}  max={np.max(r):.3f}  '
          f'frac cooler(>1.02)={np.mean(r>1.02):.2f}  frac cooler(>1.5)={np.mean(r>1.5):.3f}')
