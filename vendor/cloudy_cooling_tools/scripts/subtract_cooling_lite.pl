...
sub subtractComponents {
  ...
  TE: for (my $q = 0;$q < @{$largeTe};$q++) {
	...
	OFFSET: for (my $w = 0;$w < @{$smallTe};$w++) {
      ...
      printDebug(10,"Te: $$largeTe[$q]\n");
      # Cloudy 17 reports component strengths as fractions of the total
      # heating or cooling rate.  largeValuePtr already contains that total
      # rate normalized by the CIAOLoop cooling scale factor, so multiplying
      # the summed component fractions by it restores the physical rate.
      $scalingFactor = $largeValuePtr->[$q];
